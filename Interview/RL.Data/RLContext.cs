...
public class RLContext : DbContext
{
    ...
    protected override async void OnModelCreating(ModelBuilder builder)
    {
        ...
        {
            typeBuilder.HasKey(x => new { x.PlanId, x.ProcedureId, x.UserId });
            typeBuilder.HasOne(x => x.PlanProcedure).WithMany(pp => pp.PlanProcedureUsers).HasForeignKey(x => new { x.PlanId, x.ProcedureId });
            typeBuilder.HasOne(x => x.User).WithMany().HasForeignKey(x => x.UserId);
        });
